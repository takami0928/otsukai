...
家庭内のお使い依頼を、共有URLだけで渡せるスマホファーストのWebアプリです。依頼者は商品と条件を選び、LINE公式Web共有画面から購入リストURLを送り、お使いする人はそのURLをスマホで開いて、売り場順のリストを見ながら購入状況を記録できます。
...
依頼内容はLINE公式Web共有URLを使い、購入リストURLを `url` パラメータ、依頼本文を `text` パラメータとしてLINEへ渡します。LINE共有ページを利用できない場合に備えて、依頼文と購入リストURLをクリップボードへコピーする操作も用意しています。同じ依頼内容を再送するときは同じ購入リストURLを再利用します。

購入中の個別相談・一括相談と買い物結果は、ブラウザが対応している場合は Web Share API を使います。共有画面でLINEを選択してください。Web Share APIを利用できない場合は、相談文または結果文をクリップボードへコピーします。

LINE Messaging APIやLINEログインは使用しません。LINEで受け取った回答はアプリへ自動反映されないため、購入者が回答を確認して商品状態を変更します。

## 購入画面の売り場順

購入画面の商品カテゴリはトライアル八幡東店の売り場順で表示し、冷凍食品を最後に配置します。依頼作成画面の商品マスター順、会計前チェックの最新投入順、共有URLのpayload形式は変更しません。
